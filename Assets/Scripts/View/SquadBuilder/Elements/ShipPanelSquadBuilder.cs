...
public class ShipPanelSquadBuilder : MonoBehaviour {
    ...
    private void LoadTooltipImage(GameObject thisGameObject, string url)
    {
        ...
        {
            SetObjectSprite(thisGameObject.transform.Find("ShipImage").gameObject, TextureCache.Cache[textureCacheKey], true);
        }
    }
    ...
    private void SetObjectSprite(GameObject targetObject, Texture2D newTexture, bool textureIsScaled)
    {
        ...
        if (!TextureCache.Cache.ContainsKey(textureCacheKey)) TextureCache.Cache.Add(textureCacheKey, newTexture); //Since this Texture has been modified to fit this particular sprite we should just cache it for the ShipPanel
    ...
    {
        Global.SquadBuilder.CurrentShipName = shipName;
