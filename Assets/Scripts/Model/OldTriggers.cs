...
public static partial class OldTriggers
{
    class OldTrigger
    {
        ...
        public OldTrigger(string name, TriggerTypes triggerType, EventHandler triggerExecution, object sender, Players.PlayerNo playerNo, EventArgs arguments = null)
        {
            Name = name;
    ...
    private static Dictionary<int, OldTrigger> simultaneousTriggers = new Dictionary<int, OldTrigger>();
    private static List<Dictionary<int, OldTrigger>> stackedTriggers = new List<Dictionary<int, OldTrigger>>();

    public static void RevamedAddTrigger(string name, TriggerTypes triggerType, EventHandler triggerExecution, object sender, Players.PlayerNo playerNo, EventArgs arguments = null)
    {
        if (DebugManager.DebugTriggers) Debug.Log("Trigger \"" + name + "\" is registered. Id " + counter + ". Active: " + (simultaneousTriggers.Count+1));
        simultaneousTriggers.Add(counter, new OldTrigger(name, triggerType, triggerExecution, sender, playerNo, arguments));
    }
    ...
    public static IEnumerator RenamedResolveAllTriggers(TriggerTypes triggerType)
    {
        if (DebugManager.DebugTriggers) Debug.Log("Are triggers empty? : " + OldTriggers.Empty);
        while (!OldTriggers.Empty)
        {
            if (DebugManager.DebugTriggers) Debug.Log("I want to resolve all trigers with type: " + triggerType);

            Dictionary<int, OldTrigger> filteredTriggers = GetAllTriggersByType(triggerType);
            ...
            yield return OldTriggers.CallTrigger(triggerType);
        }
    }
    ...
    private static IEnumerator CallTriggerForPlayer(Players.PlayerNo playerNo, TriggerTypes triggerType)
    {
        ...
        Dictionary<int, OldTrigger> results = GetAllTriggersByTypeAndPlayer(playerNo, triggerType);
    ...
    private static Dictionary<int, OldTrigger> GetAllTriggersByTypeAndPlayer(Players.PlayerNo playerNo, TriggerTypes type)
    {
        var rawResults =
            ...
            select n;
        Dictionary<int, OldTrigger> results = rawResults.ToDictionary(n => n.Key, n => n.Value);

        return results;
    }

    private static Dictionary<int, OldTrigger> GetAllTriggersByType(TriggerTypes type)
    {
        var rawResults =
            from n in simultaneousTriggers
            where n.Value.TriggerType == type
    ...
    private class TriggersOrderSubPhase : DecisionSubPhase
    {
        ...
        public override void Prepare()
        {
            ...
            {
                if ((trigger.Value.Sender as Ship.GenericShip).Owner.PlayerNo == OldTriggers.CurrentPlayer)
                {
