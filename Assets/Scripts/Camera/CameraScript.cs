...
public class CameraScript : MonoBehaviour {
	...
	private void CamZoomByMouseScroll() {
		...
		if (zoom != 0) {
			Vector3 newPosition = transform.position + (transform.Find("Main Camera").TransformDirection(0, 0, zoom));
            ...
            transform.Translate (transform.InverseTransformDirection (transform.Find ("Main Camera").TransformDirection (0, 0, zoom * zoomClampRate)));
	...
	private void CamRotateByMouse() {
		if (Input.GetKey(KeyCode.Mouse1)) {
			...
			transform.Find ("Main Camera").Rotate (turnX, 0, 0);
    ...
    private float CamClampRotation(float turnX) {
		float currentTurnX = transform.Find ("Main Camera").eulerAngles.x;
