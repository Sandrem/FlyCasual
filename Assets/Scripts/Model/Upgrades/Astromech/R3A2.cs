...
namespace Abilities
{
    public class R3A2Ability : GenericAbility
    {
        ...
        private void AssignStress(Action callback)
        {
            ...
            HostShip.AssignToken(new Tokens.StressToken(HostShip), delegate { AssignStressToDefender(callback); });
        }

        private void AssignStressToDefender(Action callback)
        {
            Combat.Defender.AssignToken(new Tokens.StressToken(Combat.Defender), callback);
        }
